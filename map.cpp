...
int main() {
  ...
  double rt = 10000000, rga = 10000000; //2-6초 사이에 랜덤으로 초를 받기위한 변수, 큰 수로 초기화 하지 않으면 111번째 줄에 시작하자마자 걸리기 때문에 큰 값으로 초기화
  int err; //에러 번호
  int x, y, ga_x1, ga_y1, ga_x2, ga_y2;; //아이템 개수와 좌표
  int t_ck = 0, tga_ck = 0, gateAssign = 0;
...
//벽 : 1 , 모서리 : 2, 머리 : 3, 꼬리 : 4, growth : 5, poison : 6, gate : 7 (배열기준)
  ...
  for (int i = 0; i < 21; i++){
    for (int j = 0; j < 40; j++){ //map을 돌기 위한 2중
      if(map[i][j] == 1) wall.push_back(pair<int, int>(i,j));
    }
  }
...
void Snake::Score(){
  ...
  if (tail == TAIL) ck_tail = 'v';
  if (growth_Cnt == GROWTH) ck_grow = 'v';
  if (poison_Cnt == POISON) ck_poi = 'v';
  if (gate_Cnt == GATE) ck_gate = 'v';

  win = newwin(10, 30, 0, 50);
  mvwprintw(win, 0, 5, "ScoreBoard");
...
//game over 1 : 벽에 닿을때, 2 : 반대방향으로 갈 때, 3 : 자기 몸 통과, 4 : 꼬리 2 이하일때
  game_result = ch;
  return game_result;
}
...
void Snake::gate_xy(){
		...
		random_device rd;
	  	mt19937 gen(rd());
		shuffle(wall.begin(), wall.end(), gen); //map전체에 대한 pair를 만들어서 랜덤으로 돌리
		iter = wall.begin();
...
void Snake::is_Edge(int ny, int nx){ //가장 자리인지 확인하는 함수
	if(ny == 0 && (1 <= nx && nx <= 38)){
		Exit_gate(nx, ny, 1);
	}else if(ny == 20 && (1 <= nx && nx <= 38)){
		Exit_gate(nx, ny, 2);
	}else if((1 <= ny && ny <= 19) && nx == 0){
		Exit_gate(nx, ny, 3);
	}else if((1 <= ny && ny <= 19) && nx == 39){
		Exit_gate(nx, ny, 4);
	}else{ //가장 자리가 아닐 시에 처리하는 함수
    //1) 가장자리가 아니면서 벽인경우 (ppt에 있는 방향대로)
    //2) 가장자리가 아니면서 벽이 아닌경우 (게이트를 들어간 방향과 나오는 방향 일치)
	}
}
void Snake::Exit_gate(int x, int y, int d){
	...
	switch(d){
		case 1: //up -> down
		next_X = dir[2].x; next_Y = dir[2].y;
		cout << "case1" << endl;
		break;
		case 2: //down -> up
		next_X = dir[0].x; next_Y = dir[0].y;
		cout << "case2" << endl;
		break;
		case 3: //left -> right
		next_X = dir[1].x; next_Y = dir[1].y;
		cout << "case3" << endl;
		break;
		case 4: //right -> left
		next_X = dir[3].x; next_Y = dir[3].y;
		cout << "case4" << endl;
		break;
	}
	//////////////////이동//////////////////////////
